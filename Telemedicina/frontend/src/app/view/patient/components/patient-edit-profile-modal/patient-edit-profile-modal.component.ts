...
export class PatientEditProfileModalComponent implements  OnInit {
  ...
  ngOnInit() {
    const cleanVal = (val: any) => (val === 'N/A' || val === '' ? null : val);

    this.editProfileForm = this.fb.group({
      name: [cleanVal(this.user.user.name), [Validators.pattern(TEXT_PATTERN)]],
      address: [cleanVal(this.user.user.address), [Validators.pattern(TEXT_PATTERN)]],
      phoneNumber: [cleanVal(this.user.user.phoneNumber), [Validators.pattern(PHONE_PATTERN)]],
      homePhone: [cleanVal(this.user.patient.homePhone), [Validators.pattern(PHONE_PATTERN)]],
      height: [cleanVal(this.user.patient.height), [Validators.min(50), Validators.max(300)]],
      weight: [cleanVal(this.user.patient.weight), [Validators.min(10), Validators.max(500)]],
